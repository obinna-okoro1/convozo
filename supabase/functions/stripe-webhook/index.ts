  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // ── Call booking ────────────────────────────────────────────────────────
        ...
        if (callCreator) {
          // Build join URLs — fan URL is token-gated (no session); creator URL requires their JWT.
          const appUrl = getAppUrl();
          ...
          // 1. Confirmation to the booker — includes scheduled time and their join link
          ...
          // 2. Notification to the creator — includes scheduled time and their join link
